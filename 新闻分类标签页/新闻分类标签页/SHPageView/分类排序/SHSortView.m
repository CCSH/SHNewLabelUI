...
@interface SHSortView () <LXReorderableCollectionViewDataSource, LXReorderableCollectionViewDelegateFlowLayout>
@property (nonatomic, strong) NSMutableArray *channelList;
@end

@implementation SHSortView
...
- (instancetype)initWithFrame:(CGRect)frame channelList:(NSMutableArray *)channelList
{
	...
	if (self) {
		...
		// 右上角加个XX
		UIButton *closeBtn = [[UIButton alloc] initWithFrame:CGRectMake(WIDTH-44, 0, 44, 44)];
		...
		[collectionView registerClass:[SHSortCell_first class] forCellWithReuseIdentifier:reuseID1];
		[collectionView registerClass:[SHSortCell class] forCellWithReuseIdentifier:reuseID2];
...
{
	if (indexPath.row == 0) {
		SHSortCell_first *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseID1 forIndexPath:indexPath];

		[cell.button setTitle:_channelList[0] forState:UIControlStateNormal];
		[cell.button addTarget:self action:@selector(cellButtonClick:) forControlEvents:UIControlEventTouchUpInside];
		return cell;
	} else {
		SHSortCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseID2 forIndexPath:indexPath];
		[cell.button setTitle:_channelList[indexPath.row] forState:UIControlStateNormal];
		[cell.button addTarget:self action:@selector(cellButtonClick:) forControlEvents:UIControlEventTouchUpInside];
...
{
    NSArray *tempArr = [NSArray arrayWithArray:_channelList];
	[_channelList removeObjectAtIndex:fromIndexPath.item];
	[_channelList insertObject:tempArr[fromIndexPath.item] atIndex:toIndexPath.item];
}
